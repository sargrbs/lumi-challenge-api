...
export const extractDataFromPdf = async (
  ...
  try {
    ...
    const { clientNumber, installationNumber } =
      extractClientAndInstallationNumbers(text)
    ...
    const { dueDate, paymentCode, clientName } =
      extractDueDatePaymentCodeClientName(text)
    ...
    const invoiceDueDate = new Date(format(new Date(dueDate), 'yyyy-dd-MM'))
...
export function sumTotalConsumption(
...
export function sumTotalWithoutGD(
...
function extractClientAndInstallationNumbers(text: string): {
  clientNumber: string
...
function extractReferenceMonth(text: string): string {
  const regex =
    /Referente a\s+Vencimento\s+Valor a pagar \(R\$\)\s+(\w+\/\d{4})/
  const match = text.match(regex)
...
function extractElectricityTotal(text: string): number {
  const match = text.match(
    /Energia Elétrica\s+kWh\s+(?:[-]?[\d.,]+\s+){2}([-]?[\d.,]+)/
  )
  return match ? parseFloat(match[1].replace('.', '').replace(',', '.')) : 0
}
...
function extractEnergySceeeTotal(text: string): number {
  const match = text.match(
    /Energia SCEE s\/ ICMS\s+kWh\s+(?:[-]?[\d.,]+\s+){2}([-]?[\d.,]+)/
  )
  return match ? parseFloat(match[1].replace('.', '').replace(',', '.')) : 0
}
...
function extractEconomyGDITotal(text: string): number {
  const match = text.match(
    /Energia compensada GD I\s+kWh\s+(?:[-]?[\d.,]+\s+){2}([-]?[\d.,]+)/
  )
  return match ? parseFloat(match[1].replace('.', '').replace(',', '.')) : 0
}
...
function extractDueDatePaymentCodeClientName(text: string): {
  dueDate: string
  paymentCode: string
  clientName: string
} {
  ...
  const paymentCode = paymentCodeMatch
    ? paymentCodeMatch[1].replace(/\s+/g, '')
    : ''

  const clientNameRegex =
    /((\d+)-\d\s+(\d+)-\d\s+(\d+)-\d\s+(\d+)-\d)\s+ATENÇÃO: DÉBITO AUTOMÁTICO\s+([A-Za-zÀ-ÿ\s]+)/
  const clientNameMatch = text.match(clientNameRegex)
  const clientName = clientNameMatch
    ? extractLTDA(clientNameMatch[6])
    : extractName(text)
...
function extractName(text: string): string {
  const clientNameRegex =
    /((\d+)-\d\s+(\d+)-\d\s+(\d+)-\d\s+(\d+)-\d)\s+([A-Za-zÀ-ÿ\s]+)/
  const clientNameMatch = text.match(clientNameRegex)
